...
const Header = ({ links = [] }) => {
    ...
    const renderSublinks = (sublinks) => {
        return sublinks.map(sublink => {
            return <li key={sublink.title}><Link className="nav-link" to="/">{sublink.title}</Link></li>
        })
    }
    ...
    const renderLinkWithSublinks = (link) => {
        return (
            <li key={link.title}>
                <Link className="nav-link desktop-link" to="/">{link.title}</Link>
    ...
    const renderLinks = () => {
        ...
        return links.map(link => {
            ...
            return <li key={link.title}><Link className="nav-link" to="/">{link.title}</Link></li>
        })
    }

    return (
        <header>
            <h1 className="logo">MyPortoPage</h1>
